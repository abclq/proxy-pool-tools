...
# 代理去重参数
MAX_PORTS_PER_IP = 15  # 同一IP最多保留端口数（proxyscrape.com限制为5）
MAX_PORTS_PER_IP_PROXYSCRAPE = 5  # proxyscrape.com 的特殊限制

# 验证失败策略
MAX_VALIDATE_FAILS_BEFORE_DELETE = 5  # 失败5次即删除（与Proxy.py一致）
VALIDATE_FAIL_EXPIRE = 3  # 失败3次后延迟倍数增大

# 延迟阈值
MAX_ACCEPTABLE_LATENCY = 8000  # 超过此延迟(ms)的代理标记低质

# 延迟分层（用于API查询）
LATENCY_FAST = 3000   # <3s 快速代理
LATENCY_NORMAL = 8000  # 3-8s 普通代理
# >8s 为慢速代理

# HTTPS协议策略
HTTPS_VALIDATE_ENABLED = False  # HTTPS代理几乎全死，默认跳过验证

...
def pushNewFetch(fetcher_name, protocol, ip, port):
    ...
    port : 代理端口
    """
    from config import MAX_PORTS_PER_IP, MAX_PORTS_PER_IP_PROXYSCRAPE
    ...
    # 检查同IP端口数限制（proxyscrape.com更严格）
    port_limit = MAX_PORTS_PER_IP_PROXYSCRAPE if fetcher_name == 'proxyscrape.com' else MAX_PORTS_PER_IP
    c.execute('SELECT COUNT(*) FROM proxies WHERE ip=?', (ip,))
    ip_port_count = c.fetchone()[0]
